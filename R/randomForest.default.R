...
"randomForest.default" <-
  function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
    ...
    if (is.null(keep.subset.var)) {
      ...
      maxCard <- 0L
    } else if (is.list(keep.subset.var)) {
      stopifnot (length(keep.subset.var) == ntree)
      subsetVarCard <- as.integer(sapply(keep.subset.var, length))
      maxCard <- as.integer(max(subsetVarCard))
      subsetVar <- sapply(keep.subset.var, function(vv) {
        keep.vars <- rep(0, maxCard)
        keep.vars[1:length(vv)] <- vv
        return(as.integer(keep.vars))
    })

    } else {
      subsetVar <- rep(as.integer(keep.subset.var), times=ntree)
      subsetVar <- matrix(subsetVar, ncol=ntree)
    ...
    if (classRF) {
      ...
      rfout <- .C("regRF",
        ...
        as.integer(ntree),
        as.integer(mtry),
        selprobfull = as.double(mtry.select.prob),
        obsgini = as.double(matrix(0, nrow=nrow(x), ncol=ncol(x))),
        obsnodes = obs.nodes,
