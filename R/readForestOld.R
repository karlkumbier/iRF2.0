...
readForestOld <- function(rfobj  # a randomForest object with forest component in it
                     , X   # n x p data matrix 
...
store<-foreach(k=1:rfobj$ntree, .combine=rbind, .multicombine=TRUE, .packages='iRF')%dopar%{
   ...
   tt_out=readTreeOld(rfTree = tt, X = X, Y = Y
...
#######################################
readTreeOld <- function(rfTree # a single tree in the forest, as in the output from getTree() function
