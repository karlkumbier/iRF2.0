...
rforest <- readForest(rfobj=rf[[3]], x=X[train.id,], y=Y[train.id], 
...
The output of `iRF` is a list containing up to five entries. If `select.iter = FALSE`, outputs for each entry will be lists with one entry per iteration. 

(1) `rf.list`: a `randomForest` object.
(2) `interaction`: the interaction stability scores. The `i0` entry indicates interaction stability scores for class-$0$ leaf nodes and the `i1` entry indicates interaction stability scores for class-$1$ leaf nodes. The $+$ and $-$ indicate whether the interaction is characterized by high or low levels of the indicated feature. For instance, the interaction `1+_2+_3+` describes an interaction that is "active" when features $1, 2,$ and $3$ are all high
```{r stability}
head(fit$interaction$i0)
head(fit$interaction$i1)
```
(3) `prevalence`: the prevalence of interactions along decision paths. The `i0` entry indicates interaction prevalence for class-$0$ leaf nodes and the `i1` entry indicates interaction prevalence for class-$1$ leaf nodes. Columns of the returned data frame indicate the minimum, mean, and maximum prevalence of an interaction in the bootstrapped random forests. Since RIT searches for prevalent interactions on random forest decision paths, prevalence can be used as a refined measure of interaction importance. For instance, looking at the difference in prevalence among class$-1$ and class$-0$ nodes identifies interacitons that are frequently used on the leaf nodes for a particular class and not the other.
```{r prevalence, message=FALSE}
library(dplyr)
prev.joined <- left_join(fit$prevalence$i1, fit$prevalence$i0, by='int') %>%
...
head(prev.joined)
```
(4) `opt.k`: the number of iterations that maximizes prediction accuracy on out of bag samples.
(5) `weights`: the feature weights used to fit the random forest that maximizes prediction accuracy on out of bag samples.


The selected interactions can be visualized using simple R functions like `dotchart`:

```{r dotchart, fig.height=6, fig.width=6}
toplot <- prev.joined$diff
names(toplot) <- prev.joined$int

dotchart(rev(toplot[1:min(20, length(toplot))]), xlab='Stability Score', 
