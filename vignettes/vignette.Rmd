...
for (iter in 1:4){
  rf[[iter]] <- randomForest(x=X[train.id,], y=Y[train.id], xtest=X[test.id,]
                   , ytest=Y[test.id], mtry.select.prob = sel_prob)
...
To find prevalent sets of features and their high-order combinations used to define these nodes, use the random intersection trees (RIT) function. The following  with the following command runs RIT on all class 1 nodes:

```{r rit}
RIT(rforest$node.feature[rforest$tree.info$prediction - 1 == 1,], 
    depth=5, branch=2, n_trees=1000)
```
