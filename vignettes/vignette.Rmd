...
on the decision paths of a random forest. In particular,
...
forests, but save the forest components for further processing.
...
that encodes the splitting features and direction of inequality along the decision path of
...
(3) `interaction`: a data table of interactions and associated importance metrics. 
- `int` indicates the interaction, with $+$ and $-$ describing whether the interaction is characterized by high or low levels of the feature. For instance, the interaction `1+_2+_3+` describes an interaction that is defined by high levels of $1, 2,$ and $3$. 

- `prevalence.diff` indicates the difference in prevalence between class-$0$ and class-$1$ leaf nodes, `sta.diff` indicates the proportion of times this value is greater than 0 across bootstrap replicates. 

- `independence` indicates the difference between the prevalence of an interaction and the expected prevalence of an interaction if features were selected independently, `sta.independence` indicates the proportion of times this value is greater than 0 across bootstrap replicates. 

- `precision` indicates the difference between the proportion of class-1 observations in leaf nodes containing the interaction and the proportion of class-1 leaf nodes in leaf nodes containing subsets of the interaction, `sta.precision` indicates the proportion of times this value is greater than 0 across bootstrap replicates.

- `stability` indicates the proportion of times the interaction was recovered by RIT across bootstrap replicates.

For a more detailed description of these importance metrics, see our paper: https://arxiv.org/abs/1810.07287.
...
toplot <- fit$interaction$prevalence.diff
...
Interactions can be visualized using response surfaces, which depict the expected response as a function of interacting features. The `readForest` function uses the argument `get.split` to extract the splitting threshold from decision paths in a fitted random forest.

```{r readforest_split}
rforest <- readForest(fit$rf.list, x=X[train.id,], get.split=TRUE)
```

We use the output of `readForest` with `get.split = TRUE` to generate response surfaces as follows
```{r response_surface}
library(rgl)
plotInt(x=X[test.id,], y=Y[test.id], int='1+_2+_3+', read.forest=rforest,
        xlab='X1', ylab='X2')
rglwidget()
```
