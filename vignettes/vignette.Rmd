...
expected response as a function of interacting features. The `readForest`
function uses the argument `get.split` to extract the splitting threshold from
decision paths in a fitted random forest.

```{r readforest_split}
rforest <- readForest(fit$rf.list, x=X[train.id,], get.split=TRUE)
```

We use the output of `readForest` with `get.split = TRUE` to generate response
surfaces as follows 
```{r response_surface}
library(rgl)
