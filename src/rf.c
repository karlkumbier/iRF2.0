/*****************************************************************
  ...
  Modifications to grow weighted decision trees Sumanta Basu Jun 11, 2016.
 *****************************************************************/
...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
  ...
  int tmpcheck, subsetCard, *subsetVarjb;
  ...
  subsetVarjb =   (int *) S_alloc(*maxCard, sizeof(int));
  ...
  for (jb = 0; jb < Ntree; jb++) {
    ...
    /* set forced selections for current tree*/
    subsetCard = subsetVarCard[jb];
    if (subsetCard > 0) {
      for (j = 0; j < subsetCard; j ++) {
        subsetVarjb[j] = subsetVar[j + *maxCard * jb];
      }
    }
  ...
  // end calculation for jb-th tree
  PutRNGstate();
