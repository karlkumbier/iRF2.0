individual_paths_frequent_feature_interactions <- function(irf_output, data, sample_index, verbose = F){
  ...
  require('arulesViz')
  # read last forest
  rf_last <- readForest(tail(irf_output$rf.list, n = 1)[[1]], x=data, return.node.obs=TRUE)
